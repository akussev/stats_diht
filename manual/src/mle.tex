...
\begin{proof}
    ...
    \[
    H(\mathsf{P}, \mathsf{P}) - H(\mathsf{P}, \mathsf{Q}) = \int p \ln{\frac{q}{p}}\,d\mu \le \int p \left(\frac{q}{p} - 1\right)\,d\mu = \int p \,d\mu - \int q \,d\mu = 1 - 1 = 0.
    \]

    Причём по свойству интеграла Лебега равенство верно, когда $\mu$-п.н. $\ln{q/p} \ge q/p - 1$, то есть $q/p = 1$, что и требовалось.
\end{proof}
