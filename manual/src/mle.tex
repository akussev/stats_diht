...
Данный метод был широко популяризирован известным статистиком Рональдом Фишером и обладает множеством любопытных свойств. Но для того, чтобы они имели место, нужно потребовать выполнения некоторых условий в нашей модели, поэтому сначала поговорим о мотивации такой оценки и рассмотрим примеры.
...
Заметим, что точки максимума функции правдоподобия и её логарифмического брата-близнеца совпадают в силу монотонности логарифма, поэтому максимизировать можно любую из этих функций. Второй вариант обычно предпочтительнее, так как максимизировать сумму гораздо проще, чем произведение.
...
\begin{definition}
    Пусть $\mathsf{Q}$ --- вероятностная мера с обобщённой плотностью $q$, а $\mathsf{P}$ --- произвольное распределение. Введём величину, называемую \textit{кросс-энтропией}:
    \[
    H(\mathsf{P}, \mathsf{Q}) = -\int \ln{q(x)}\,\mathsf{P}(dx). 
    \]
\end{definition}

Мы называем её <<расстоянием>>, потому что эта функция не является метрикой (она не симметрична и часто отрицательна), однако она хорошо показывает похожесть распределений. Посему логично рассмотреть функционал
\[
G(\mathsf{P}) = \argmin_{\theta \in \Theta} H(\mathsf{P}, \pth)
\]

Если $\mathsf{P} = \pth$, то функционал будет равен $\theta$, что гарантирует следующее
...
\begin{proposition}
    ...
    \[
    H(\mathsf{P}, \mathsf{P}) \le H(\mathsf{P}, \mathsf{Q})
    \]
...
\begin{proof}
    ...
    \[
    \ln{\frac{q}{p}} = \ln{\left(1 + \left(\frac{q}{p} - 1\right)\right)} \le \frac{q}{p} - 1.
    \]

    Значит, домножая его на $p$ и навешивая знак интеграла, получаем:
    \[
    H(\mathsf{P}, \mathsf{P}) - H(\mathsf{P}, \mathsf{Q}) = \int p \ln{\frac{q}{p}}\,d\mu \le \int p \left(\frac{q}{p} - 1\right)\,d\mu = \int q \,d\mu - \int p \,d\mu = 1 - 1 = 0.
    \]

    Причём по свойству интеграла Лебега равенство верно, когда $\mu$-п.н. $\ln{q/p} = q/p - 1$, то есть $q/p = 1$, что и требовалось.
\end{proof}

По методу подстановки оценка для сего функционала может быть получена как
\[
G(\mathsf{P}^*_n) = \argmin_{\theta\in\Theta} H(\mathsf{P}^*_n, \pth) = \argmax_{\theta\in\Theta} \int \ln{\rho_{\theta}(x)}\,\mathsf{P}^*_n(dx) = \argmax_{\theta\in\Theta} \frac{1}{n}\sum_{i=1}^n \ln{\rho_{\theta}(X_i)},
\]
...
\begin{example}[оценка Ходжеса]
    ...
    \[
    \sqrt{n}(\theta^* - \theta) = \sqrt{n}(\overline{\mathbf X} - \theta) - \overline{\mathbf X} \cdot \underbrace{\frac{\sqrt{n}}{2}\cdot I(|\overline{\mathbf X}| < n^{-1/4})}_{\stackrel{d}{\to}0} \stackrel{d}{\to} \mathcal{N}(0, 1). 
    \]
...
\[
-i(\boldtheta)_{ij} = \me[\boldtheta] \frac{\partial^2 \ln \rho_{\boldtheta}(X_1)}{\partial \theta_i \partial \theta_j} \stackrel{\boldtheta\text{-п.н.}}{\longleftarrow} \frac{1}{n}\sum_{l=1}^n \frac{\partial^2 \ln \rho_{\boldtheta}(X_l)}{\partial \theta_i \partial \theta_j} = \frac{\partial^2}{\partial \theta_i \partial \theta_j}\sum_{l=1}^n \frac{\ln \rho_{\boldtheta}(X_l)}{n},
\]
...
\[
\theta_{t+1} = \theta_{t} - [d^2L_{\theta_t}]^{-1} \cdot dL_{\theta_t}^T,
\]
где $dL_{\theta_t}^T$ --- столбец из частных производных $L_{\theta_t}(\mathbf X)$, а $d^2L_{\theta_t}$ --- матрица из вторых производных (так называемая матрица Гессе).

Неочевидным остаётся то, на сколько шагов запускать сей алгоритм. Оказывается, при некоторых условиях достаточно совершить всего лишь один шаг, чтобы получить не просто хорошую оценку, а асимптотически эффективную, то есть с такой же асимптотической дисперсией, как и у ОМП.
\begin{theorem}{}{}
...
Рассмотрим другой распространённый метод, который не только значительно упрощает нахождение решений уравнения правдоподобия, но и позволяет <<выжимать>> из данных больше информации. Данная тема заслуживает отдельной главы, однако мы затронем её лишь вскользь; более подробное её описание и больше примеров можно найти в оригинальной статье \cite{dempster1977maximum} или более обзорно, например, в \cite{murphy2022probabilistic}.
...
Теперь, когда на сцене появилось мат. ожидание, можно применить неравенство Йенсена и перейти к нижней оценки лог. функции правдоподобия, которую удобнее максимизировать.
\[
...
Мат. ожидание справа сокращённо называют ELBO (англ. \textsf{evidence lower bound}). Эта нижняя оценка обладает важным свойством: при $\boldtheta = \boldtheta_t$ в неравенстве выше достигается равенство. Действительно,
\[
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item ($E$-шаг, от слова \textsf{Expectation}) Считаем мат. ожидание
		\[
...
\begin{example}
	...
	\[
	\pth[\boldmu_t, \boldsigma_t, \pi_t](Z_i = j | X_i = x_i) \propto \pth[\boldmu_t, \boldsigma_t, \pi_t](Z_i = j, X_i = x_i) = \pi_{t,j} \cdot \phi_{\mu_{t,j}, \sigma_{t,j}^2}(x_i), \;\;\; j \in \{0, 1\}.
	\]
...
\begin{example}[робастная регрессия]
	...
	Согласно E-шагу, нам нужно найти мат. ожидание его логарифма по $\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)$ при фиксированных видимых переменных $\mathbf Y$. По формуле Байеса,
	\[
	\rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y) \propto \rho_{\boldtheta_t, \sigma_t}(\mathbf y,  \mathbf z) \propto \prod_{i=1}^n \underbrace{z_i^{k/2-1/2} \exp\left[-z_i \cdot \left(\frac{(y_i-\mathbf{x}_i^T \boldtheta_t)^2}{2\sigma_t^2} + \frac{k}{2}\right)\right]}_{\sim \Gamma((k+1)/2, (y_i-\mathbf{x}_i^T \boldtheta_t)^2 /(2\sigma_t^2)+k/2)}.
	\]
	Находим искомое мат. ожидание, одновременно скидывая в константу $C$ всё, что не зависит от значений $(\boldtheta, \sigma)$:
	\[
	\me[\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)] \ln \rho_{\boldtheta, \sigma}(\mathbf y, \mathbf z) = \underbrace{\sum_{i=1}^n \me[\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)]\ln \rho(z_i)}_{=\textrm{const}} + \sum_{i=1}^n \me[\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)] \ln \rho_{\boldtheta, \sigma}(y_i | z_i) = 
	\]
	\[
	= C + \sum_{i=1}^n \me[\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)] \left(\ln{\sqrt{z_i}} - \ln{\sqrt{2\pi \sigma^2}} - z_i \cdot \frac{(y_i-\mathbf{x}_i^T \boldtheta)^2}{2\sigma^2} \right) =
	\]
	\[
	= C' - n\ln{\sqrt{2\pi \sigma^2}} - \sum_{i=1}^n \frac{(y_i-\mathbf{x}_i^T \boldtheta)^2}{2\sigma^2} \cdot \me[\mathbf z \sim \rho_{\boldtheta_t, \sigma_t}(\mathbf z | \mathbf y)] z_i =
	\]
	\[
